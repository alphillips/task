...
class MailsList extends React.Component {
  ...
  createNewMessageThread = () => {
    if (
      ...
      this.state.messageSubject.trim().length > 0 &&
      ...
      <div className="task-detail">
        {!this.state.showMail && (
          <div style={{ paddingTop: "1em" }}>
            <span style={{ fontSize: "1.2em", fontWeight: "bold" }} className="notesicon">
              External Messages <span className="mail-link" /> <span>({this.state.messageThreads && this.state.messageThreads.length})</span>{" "}
            </span>
                ...
                <div className="inbox">
                  <PathwayList>
                    {this.state.messageThreads.map(mail => (
                      <li className={"inbox-listing " + (!mail.read ? "unread" : "")} key={mail.corroId + mail.notificationId}>
                        <div className="border-unread" />
                        <Link onClick={this.setupMail.bind(null, mail.notificationId, mail.type, mail.description)}>
                          <span className="inbox-date">{this.epochSecondToDate(mail.createdTimeStamp.epochSecond)}</span>
                          {this.state.attachment && <span className="inbox-attachment" />}
                          <span className="inbox-from">{mail.fromParty}</span>
                          <span className="inbox-subject">{mail.description}</span>
